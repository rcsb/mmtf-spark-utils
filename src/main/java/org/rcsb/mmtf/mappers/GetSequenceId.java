...
public class GetSequenceId implements PairFunction<Tuple2<Integer,Integer>,String,Float> {
	...
	private AminoAcidCompoundSet totalCmpdSet;
	...
	public GetSequenceId(Broadcast<List<Tuple2<String,CalphaAlignBean>>> inputData) {
		...
		totalCmpdSet = AminoAcidCompoundSet.getAminoAcidCompoundSet();
	}
	/**
	 * 
	 */
	...
	public Tuple2<String, Float> call(Tuple2<Integer, Integer> inputTuple) throws Exception {
		...
		PairwiseSequenceAligner<ProteinSequence, AminoAcidCompound> smithWaterman = Alignments.getPairwiseAligner(ps1, ps2, PairwiseSequenceAlignerType.GLOBAL, new SimpleGapPenalty(), matrix);
		...
		if(smithWaterman.getSimilarity()>0.0){
			System.out.println(smithWaterman.getSimilarity());
		}
