...
//		pdbCodeList.add("3DBQ");
		...
		String uri = "/data/outTotFile.hadoop";
	...
	private Structure genBioJavaStruct(String inputId) throws IOException, StructureException{
		AtomCache cache = new AtomCache();
		cache.setUseMmCif(true);
		...
		params.setParseBioAssembly(true);
		StructureIO.setAtomCache(cache);
		Structure bioJavaStruct = StructureIO.getStructure(inputId);
		...
		try{
			...
			e.printStackTrace();
			DSSPParser.fetch(inputId, bioJavaStruct, true); //download from PDB the DSSP result
		}
		return bioJavaStruct;
